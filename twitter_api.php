<?php
	...
	for($i = 0; $i < $count; $i++)
	{
		...
		if(!$skip)
		{
			...
			// Extract tweetid
			$find	=	$finder->query("//*[contains(concat(' ', normalize-space(@class), ' '), 'js-stream-item')]");
			$data	.=	'"tweetid":"' . htmlspecialchars($find->item(0)->nodeValue, ENT_QUOTES) . '",';
